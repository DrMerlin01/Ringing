...
//*****************************************************************************
void lcd_dat(uint8_t ch)		//Вывод символов на дисплей
{
	LDP=(ch&0b11110000);//
	LCP|=1<<LCD_RS;//выбираем регистр данных
	LCP|=1<<LCD_E;	//	стробирующий импульс
	_delay_ms(1);
	LCP&=~(1<<LCD_E);//	сбрасываем стробирующий импульс
	LCP&=~(1<<LCD_RS);//устанавливаем регситр команд
	_delay_ms(1);
	LDP=((ch&0b00001111)<<4);//
	LCP|=1<<LCD_RS;//регситр данных
	LCP|=1<<LCD_E;	//	стробирующий импульс в 1
	_delay_ms(1);
	LCP&=~(1<<LCD_E);//	импульс в 0
	LCP&=~(1<<LCD_RS);//регистр команд
	_delay_ms(1);
}
//*****************************************************************************
void lcd_com(uint8_t cmd)	//Отправляет команду на дисплей
{
	LDP=(cmd&0b11110000);//
	LCP|=1<<LCD_E;		//устанавливаем стробирующий импульс в 1
	_delay_ms(1);
	LCP&=~(1<<LCD_E);//сбрасываем стробирующий импульс
	_delay_ms(1);
...
{
	lcd_com(1<<LCD_CLR);//устанавливаем бит PA0 в лог. 1
}
//****************************************************************************
...
void lcd_gotoxy(uint8_t x, uint8_t y)	// Переместить курсор в позицию XY
{
	...
	// set data address
	lcd_com(1<<LCD_DDRAM | DDRAMAddr);//устанавливаем адрес
}
//*****************************************************************************
