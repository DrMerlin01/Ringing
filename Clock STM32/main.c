...
void TIM6_DAC_IRQHandler(void)//звуковое оповещение о перерывах и занятиях
{
	...
	if ((hours==8 && minute==0 && second<=15)||(hours==12 && minute==30 && second<=15))
	{
		if (pisk==0)
		{
	...
	else if ((hours==13 && minute==00 && second<=15)||(hours==18 && minute==00 && second<=15))
	{
		if (pisk==0)
		{
			GPIO_SetBits(GPIOB, GPIO_Pin_15);
